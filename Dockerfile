...
RUN cd /opt/quiche-src && \
    CXXFLAGS="-isystem $(dirname "$(dirname "$(find /usr/include/c++ -name c++config.h -print -quit)")")" RUST_BACKTRACE=1 PATH="$HOME/.cargo/bin:$PATH" RUSTFLAGS="-C target-feature=-crt-static" cargo build -vv --package quiche --release --features ffi,pkg-config-meta,qlog --jobs $(nproc) && \
...
RUN cd /opt/curl-src && \
    autoreconf -fi && \
    ./configure LDFLAGS="-Wl,-rpath,/usr/local/lib" LIBS="-lstdc++" --with-openssl=/opt/quiche-src/quiche/deps/boringssl/src --with-quiche=/opt/quiche-src/target/release --with-nghttp2 --with-zlib && \
    make -j $(nproc) && \
...
FROM alpine:edge
RUN apk add --no-cache nghttp2 zlib libpsl libstdc++ bash perl ca-certificates
